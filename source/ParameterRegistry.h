...
public:
  ...
  void Init(const float control_rate) { control_rate_ = control_rate; }

  void Register(const ParamId id,
                const float initial_value,
                const float min,
                const float max,
                Handler handler,
                const float smooth_time = 0.05f,
  ...
  // Called in response to new control input. Updates target, does not apply
  // value.
  void Update(const ParamId id,
              const float value,
  ...
  void UpdateNormalized(const ParamId id,
                        const float normValue,
                        const bool immediate = false) {
    auto it = param_states_.find(id);
    if (it != param_states_.end()) {
      auto& state = it->second;
      auto mappedValue = daisysp::fmap(daisysp::fclamp(normValue, 0.0f, 1.0f),
...
private:
  ...
  struct ParamState {
    ...
    ParamState() = delete;
    ParamState(const float initial,
               const float min,
               const float max,
               const float coef,
               const daisysp::Mapping mapping,
